...
void MakingReservation::chooseArrivalAirport( int departureAirport, int &arrivalAirport )
{
    ...
    do 
    {
        ...
        cout << "? ";
        cin >> arrivalAirport;
    } while (!canArrive[arrivalAirport]);
}
...
void MakingReservation::displayReservation( Reservation reservation )
{
    ...
    cout << right << setw(9) << flightSchedule.getDepartureTime(reservation.getFlightNo())
        << "    " << left << setw(9) << flightSchedule.getArrivalTime(reservation.getFlightNo()) << endl << endl;

    int total = 0;
    for (int i = 1; i < 8; i++)
    {
        if (reservation.getTicket(i))
        {
            int fare = fullFare[depart][arrive] * discount[i] / 100;
            cout << right << setw(23) << ticketType[i] << "  TWD " << setw(4)
                << fare << " x " << reservation.getTicket(i) << endl;
