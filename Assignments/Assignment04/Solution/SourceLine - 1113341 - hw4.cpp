...
{
    //size_t length = sourceLine.size();
    size_t strStart{ sourceLine.find('"') }, strEnd{};
    while (strStart != sourceLine.npos) /*if not find will return npos*/
    {
        //cout << sourceLine << endl;
        strEnd = sourceLine.find('"', strStart + 1);
        //cout << "SE1:" << strEnd << endl;

        /*if (strEnd == sourceLine.npos) /*if more then two strConst and the sourceLine has been resized - test1 line15
         strEnd = sourceLine.size() - 1;*/

        while (sourceLine[strEnd - 1] == '\\') /*avoid \"*/
            strEnd = sourceLine.find('"', strEnd + 1);

        //cout << "SE2:" << strEnd << endl;
        sourceLine.erase(strStart, strEnd - strStart + 1);/*total delete (strEnd - strStart + 1) character*/
        //cout << sourceLine.size() << endl;
        //cout << sourceLine << endl << endl;
        strStart = sourceLine.find('"');/*find the rest of strConst*/
        //cout << "SS:" << strStart << endl;
    }
...
{
    size_t length = sourceLine.size();
    size_t charStart{ sourceLine.find('\'') }, charEnd{};
    //cout << "indelChar\n";
    //cout << endl << "cSize: " << sourceLine.size() << endl;
    while (charStart != sourceLine.npos)
    {
        //cout << sourceLine << endl;
        charEnd = sourceLine.find('\'', charStart + 1);
        while (sourceLine[charEnd - 1] == '\\')
            charEnd = sourceLine.find('\'', charEnd + 1);

        sourceLine.erase(charStart, charEnd - charStart + 1);
        //cout << sourceLine << endl << endl;
        charStart = sourceLine.find('\'');
    };
