...
public:
    ...
    iterator insert( const_iterator where, const value_type &val )
    {
        ...
        if( originalSize == originalCapacity )
        {
            ...
            myData.myFirst[here] = val;
            for (size_type i = here + 1; i <= originalSize; i++)
        ...
        else
        {
            ...
            size_type total = myData.myLast - myData.myFirst, here = where - myData.myFirst;
            for (size_type i = total; i > here; i--)
                *(p + i) = *(p + i - 1);
            *(p + here) = val;
    ...
    iterator erase( const_iterator where )
    {
        if( where < myData.myFirst || where >= myData.myLast )
            return nullptr;
        size_type here = where - myData.myFirst;
        for (size_type i = here; i < myData.myLast - myData.myFirst; i++)
...
bool operator==( const vector< Ty > &left, const vector< Ty > &right )
{
    ...
    for (size_t i = 0; i < right.size(); i++)
        if (left.begin()[i] != right.begin()[i]) /*why can left[i], why cannot left.myData.myFirst[i]*/
            return false;
