...
public:
    ...
    vector& operator=( const vector &right )
    {
        if( this != &right ) // avoid self-assignment
        {
            ...
            for (iterator p = myData.myFirst, r = (iterator)right.begin(); r < right.end();)
                *p++ = *r++;
    ...
    iterator erase( const_iterator where )
    {
        ...
        {
            size_type here = where - myData.myFirst;
            for (iterator p = myData.myFirst + here; p < end();)
                *p = *(++p); /*cannot *p++*/
...
public:
    ...
    bool operator<( HugeInteger &right )
    {
        ...
        typename T::iterator it = integer.end() - 1, it2 = right.integer.end() - 1;
        for (; it >= integer.begin(); it--, it2--)
        {
            if (*it < *it2)
                return true;
            else if (*it > *it2)
    ...
    HugeInteger operator-( const HugeInteger &op2 )
    {
        ...
        for (itDif = difference.integer.begin(); itDif < difference.integer.end() - 1; itDif++)
        {
            if (*(itDif) < 0 || *(itDif) > 9)
            {
                *itDif += 10;
        ...
        while (difference.leadingZero())
        {
            ...
            difference.integer.erase(itDif--);
        }
    ...
    HugeInteger operator*( HugeInteger &op2 )
    {
        ...
        for (size_t i = 0; i < op2.integer.size(); i++)
        {
            ...
            {
                product.integer.begin()[i + j] += integer.begin()[j] * op2.integer.begin()[i];
            }
        }*/
        ...
        for (++itBuf; it2 != op2.integer.end(); it2++, itBuf++)
        {
            for (it = integer.begin(); it != integer.end(); it++, itPd++)
            {
                ...
                *itPd += *it * *it2;
                //cout << *it << " " << *it2 << " " << *itPd << endl;
            }
            itPd = itBuf;
        }

        for (itPd = product.integer.begin(); itPd != product.integer.end() - 1; itPd++)
        {
            if (*itPd > 9)
            {
                (*(itPd + 1)) += (*itPd) / 10;
    ...
    HugeInteger operator/( HugeInteger &op2 )
    {
        ...
        typename T::iterator itBuf = buffer.integer.end() - 1, it2 = op2.integer.end() - 1;
        for (; it2 >= op2.integer.begin(); itBuf--, it2--)
        {
            *itBuf = *it2;
        }
        ...
        /*cout << "1re: " << remainder << endl;
        cout << "1buf: " << buffer << endl;*/
        typename T::iterator itQuo = quotient.integer.end() - 1;
        for (; itQuo >= quotient.integer.begin(); itQuo--)
        {
            while (buffer <= remainder)
            {
                remainder -= buffer;
                /*cout << remainder.size() << " re: " << remainder << endl;
                cout << buffer.size() << "buf: " << buffer << endl;*/
                (*itQuo)++;
                if (remainder.isZero())
                    return quotient;
            }
    ...
    void convert( const value_type &number )
    {
        ...
        cout << integer.end() - integer.begin();
    }
