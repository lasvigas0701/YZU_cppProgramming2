...
public:
        ...
        : myData()
    {
        ...
        else
        {
            ...
            size_type rowNum = (count - 1) / dequeSize + 1;

            /*same as 374 ~ 378
            for (size_type i = 0; i < (count - 1) / dequeSize + 1; i++)
                myData.map[i] = new value_type [dequeSize]();
            
            for (size_type i = 0; i < count; i++)
            {
                size_type row, col;
                row = (i / dequeSize) % myData.mapSize; col = i % dequeSize;
                myData.map[row][col] = val;
            }*/

            for (MapPtr itMap = myData.map; itMap < myData.map + rowNum; itMap++)
                *itMap = new value_type[dequeSize]();

            for (iterator it = makeIter(begin()); it < begin() + count; it++)
                *it = val;
        }
