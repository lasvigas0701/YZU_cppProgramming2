...
public:
    ...
    iterator insert( const_iterator where, const value_type &val )
    {
        ...
        {//cout << ":D: " << off;
            ...
            myData.myOff = (off - 1) % (myData.mapSize * dequeSize); /*why need -1*/
            myData.map[getBlock(myData.myOff)] = new value_type[dequeSize]();
            myData.map[myData.myOff / dequeSize][myData.myOff % dequeSize] = val;
            
            //cout << "off: " << (off - 1) % (myData.mapSize * dequeSize);
            //cout << endl << "---" << endl;
        }
        else
        {
            ...
            if( off <= myData.mySize / 2 )
            {
                ...
                Ty* temp = new value_type[myData.mySize + 1](); /*a corresponding 1-Dimension array*/
               
                iterator it = begin();
                size_type i = 0;
                
                for (; it != begin() + off; i++, it++)
                    temp[i] = *it; /*temp[0] is the data of begin()*/
                temp[i++] = val;
                for (; it != end(); i++, it++)
                    temp[i] = *it;

                myData.myOff = (myData.myOff - 1) % (myData.mapSize * dequeSize);/*myData.myOff--, avoid -1*/
                
                if (myData.myOff % dequeSize == dequeSize - 1) /*if new begin() is in the previous row*/
                    myData.map[getBlock(myData.myOff)] = new value_type[dequeSize]();
                
                /*current begin() is the previous index of original*/
                for (it = begin(), i = 0; it != end(); it++, i++) /*put datas back to deque*/
                    *it = temp[i];
            ...
            else  // all elements after (and at) where move backward
            {
                ...
                Ty* temp = new value_type[myData.mySize + 1]();
                ...
                if (myData.map[getBlock(myData.myOff + myData.mySize + 1)] == nullptr)
                    myData.map[getBlock(myData.myOff + myData.mySize + 1)] = new value_type[dequeSize]();

                /*end() + 1 because the number of datas + 1, add at the end*/
                for (it = begin(), i = 0; it != end() + 1; i++, it++)
                    *it = temp[i];
                /*cout << "mapSz: " << myData.mapSize << endl;*/
...
//      return ( ( myData.off / compDequeSize() ) & ( myData.mapSize - 1 ) );
    ...
    void doubleMapSize()
    {
        if( myData.mapSize > 0 )
        {
            ...
            size_type total{ 0 }, startRow = myData.myOff / dqSize; /*start from which row*/
            
            for (size_type i = 0; i < oldMapSize; i++) /*count numbers of not nullptr*/
                if (myData.map[i] != nullptr)
                    total++;
            ...
            /*copy datas to newMap from begin() to the last index of original deque or the end of datas*/
            ...
            /*if cycle, copy datas to newMap from [0][0] to the end of datas*/
