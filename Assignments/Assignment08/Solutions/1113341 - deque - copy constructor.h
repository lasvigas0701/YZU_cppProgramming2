...
public:
        ...
        : myData()
    {
        if( right.myData.mySize > 0 )
        {
            ...
            /*same as 368~374
            for (size_type i = 0; i < (myData.mySize - 1) / dequeSize + 1; i++)
                myData.map[i] = new value_type[dequeSize]();

            for (size_type i = 0, j = right.myData.myOff; i < myData.mySize; i++, j++)
            {
                /*why need % myData.mapSize
                size_type row = (i / dequeSize) % myData.mapSize, col = i % dequeSize;
                size_type rrow = (j / dequeSize) % right.myData.mapSize, rcol = j % dequeSize;
                myData.map[row][col] = right.myData.map[rrow][rcol];
            }*/
            size_type rowNum = (myData.mySize - 1) / dequeSize + 1;
            for (MapPtr p = myData.map; p < myData.map + rowNum; p++)
