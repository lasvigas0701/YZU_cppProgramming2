...
public:
    ...
    void push_front( const value_type &val )
    {
        ...
        else
        {
            ...
            myData.myOff = (myData.myOff - 1) % (dequeSize * myData.mapSize);
            iterator it = begin();
            if (myData.map[getBlock(it.myOff)] == nullptr)
                myData.map[getBlock(it.myOff)] = new value_type[dequeSize]();
            *it = val;
        }
        myData.mySize++;
    }
...
//      return ( ( myData.off / compDequeSize() ) & ( myData.mapSize - 1 ) );
    ...
    void doubleMapSize()
    {
        if( myData.mapSize > 0 )
        {
            ...
            size_type i = startRow, j = startRow;
            MapPtr p = myData.map, np = newMap;
            for (size_type count = 0; count < oldMapSize; count++, i++, j++)
            {
                if (j == oldMapSize)
                    j = 0;
                *(np + i) = *(p + j);
