...
public:
   ...
   void resize( size_type newSize )
   {
      ...
      else if( newSize > myData.mySize )
      {
         ...
         while( newSize > dequeSize * newMapSize )
            newMapSize *= 2;

         if( ( myData.myOff % dequeSize == 0 &&
               newSize > dequeSize * ( newMapSize - 1 ) && myData.mySize <= dequeSize * ( newMapSize - 1 ) ) ||
             ( myData.myOff % dequeSize > 0 && myData.myOff % dequeSize + newSize > dequeSize * newMapSize ) )
            newMapSize *= 2;
