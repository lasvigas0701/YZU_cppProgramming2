...
public:
    ...
    iterator insert( const_iterator where, const value_type &val )
    {
        ...
        if( myData.mySize == 0 )
        {
            ...
            myData.map[getBlock(myData.myOff)][myData.myOff % dequeSize] = val;
        }
        else
        {
            ...
            if( off <= myData.mySize / 2 )
            {
                ...
                if (myData.map[getBlock(myData.myOff)] == nullptr)
                    myData.map[getBlock(myData.myOff)] = new value_type[dequeSize]();
                iterator it = makeIter(begin());

                /* results same as line 449 ~ 451
                ...
                for (it = makeIter(begin()), i = 0; it != end();)
                    *it++ = temp[i++];
            ...
            else  // all elements after (and at) where move backward
            {
                ...
                if (myData.map[getBlock(end().myOff)] == nullptr)
                    myData.map[getBlock(end().myOff)] = new value_type[dequeSize]();

                /*results same as line 481 ~ 484
                iterator it = makeIter(begin());
                size_type i = 0;
                Ty* temp = new value_type[myData.mySize + 1]();
                for (; it != begin() + off; it++, i++)
                ...
                for (it = begin(), i = 0; it != end() + 1;)
                    *it++ = temp[i++];

                delete[] temp;*/

                iterator it = makeIter(end());
                for (; it != begin() + off; it--)
                    *it = *(it - 1);
                *it = val;
            }
...
//      return ( ( myData.off / compDequeSize() ) & ( myData.mapSize - 1 ) );
    ...
    void doubleMapSize()
    {
        if( myData.mapSize > 0 )
        {
            ...
            MapPtr p = myData.map, np = newMap;
            size_type i = getBlock(begin().myOff),
                j = getBlock(begin().myOff);

            for (size_type times = 0; times < oldMapSize; times++, i++, j++)
            {
                if (j == oldMapSize) j = 0;
                *(np + i) = *(p + j);
            }

            /* results same as 529 ~ 533
            size_type startRow = getBlock(begin().myOff);
            for (size_type i = startRow; i < oldMapSize; i++, j++)
                newMap[j] = myData.map[i];

             /*if cycle, copy addresses from map[0] to following row of newMap
            for (i = 0; i < startRow; i++, j++)
                newMap[j] = myData.map[i];*/
