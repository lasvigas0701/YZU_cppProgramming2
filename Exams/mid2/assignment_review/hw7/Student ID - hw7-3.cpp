...
public:
    ...
    iterator insert( const_iterator where, const Ty &val )
    {
        if( where.ptr >= myData.myFirst && where.ptr <= myData.myLast )
        {
            ...
            iterator it = myData.myFirst; const_iterator it2 = temp;
            for (; it2 < where; it++, it2++)
                *it = *it2;
            *it++ = val;
...
bool operator==( const vector< Ty > &left, const vector< Ty > &right )
{
    ...
    typename vector<Ty>::const_iterator it = left.begin(), itr = right.begin();
    for (; it != left.end(); it++, itr++)
        if (*it != *itr)
            return false;
...
public:
        ...
        : myData()
    {
        ...
        iterator temp = myData.myHead;
        for (; count--; temp++)
        {
            temp.ptr->next = new node;
            temp.ptr->next->myVal = Ty();
            temp.ptr->next->prev = temp.ptr;
            temp.ptr->next->next = myData.myHead;
        }
        myData.myHead->prev = temp.ptr;
    }
        ...
        : myData()
    {
        ...
        for (; itr!=right.end(); it++, itr++)
        {
            ...
            it.ptr->next->myVal = *itr;
            it.ptr->next->prev = it.ptr;
            it.ptr->next->next = myData.myHead;
        }
    ...
    iterator insert( const_iterator where, const Ty &val ) // insert val at where
    {
        ...
        add->myVal = val;
        add->prev = here.ptr->prev; add->next = here.ptr;
        here.ptr->prev->next = add; here.ptr->prev = add;
        
        myData.mySize++;
        return here.ptr->prev;
    }

    iterator erase( const_iterator where )
    {
        iterator del = where.ptr->next->prev;
...
bool operator==( const list< Ty > &left, const list< Ty > &right )
{
    ...
    typename list<Ty>::const_iterator it = left.begin(), itr = right.begin();
    for (; it != left.end(); it++, itr++)
        if (*it != *itr)
            return false;
...
public:
    ...
    HugeInteger operator-( const HugeInteger &op2 )
    {
        ...
        for (itDif = difference.integer.begin(); itDif != difference.integer.end(); itDif++)
        {
            if (*itDif > 9 || *itDif < 0)
            {
                typename T::iterator itBuf = itDif;
                *itDif += 10;
                (*(++itBuf))--;
            }
        }
    ...
    HugeInteger operator*( HugeInteger &op2 )
    {
        ...
        HugeInteger product(size() + op2.size());
        typename T::iterator it = integer.begin(), it2 = op2.integer.begin();
        typename T::iterator itPro = product.integer.begin(), itBuf = itPro;
        
        for (++itBuf; it2 != op2.integer.end(); it2++, itBuf++)
        {
            for (it = integer.begin(); it != integer.end(); it++, itPro++)
                *itPro += *it * *it2;
            itPro = itBuf;
        }

        for (itPro = product.integer.begin(); itPro != product.integer.end(); itPro++)
        {
            if (*itPro > 9)
            {
                itBuf = itPro;
    ...
    HugeInteger operator/( HugeInteger &op2 )
    {
        ...
        if( *this < op2 )
            return zero;
        size_type size = integer.size() - op2.size();
        ...
        /*cout << "First re: " << remainder << endl;
        cout << "First buf: " << buffer << endl;*/
